...
curl -skSL \
    ...
    jq -er '[.completed_frameworks[] | select(.role != "*") | {id: .id, name: .name, role: .role, principal: .principal, active: .active, unregistered_time: .unregistered_time, completed_tasks: ([.completed_tasks[] | {name: .name, framework_id: .framework_id, slave_id: .slave_id, resources: .resources}] | sort_by(.name))}] | sort_by(.name)' | \
